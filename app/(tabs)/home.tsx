...
export default function HomeScreen() {
  ...
  const unreadNotifications = notifications.filter((item) => !item.opened);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text style={styles.statusSubtitle}>
          {isSafe
            ...
            : `${activeIncidents.length} active incidents require attention, ${openTasks.length} tasks pending, ${unreadNotifications.length} unread notifications.`}
        </Text>
      </View>
      ...
      <View style={styles.actionGrid}>
        <ActionCard
          title="Map View"
          icon={<Navigation color="#0f172a" size={24} />}
          onPress={() => router.push("/(tabs)/map")}
        />
        <ActionCard
          title="AI Assistant"
          icon={<Activity color="#0f172a" size={24} />}
          onPress={() => router.push("/assistant")}
        />
        <ActionCard
          title="Safety Tips"
          icon={<ShieldCheck color="#0f172a" size={24} />}
          onPress={() => router.push("/safety-tips")}
        />
        <ActionCard
          title="Contacts"
          icon={<MapPin color="#0f172a" size={24} />}
          ...
          <View style={styles.incidentTop}>
            ...
            <View
              style={[
                ...
                {
                  backgroundColor:
                    inc.status !== "Resolved" ? "#fee2e2" : "#f1f5f9",
                },
              ]}
            >
              <Text
                style={[
                  styles.badgeText,
                  { color: inc.status !== "Resolved" ? "#dc2626" : "#64748b" },
                ]}
              >
                {inc.status}
              </Text>
...
const styles = StyleSheet.create({
  ...
  statusTitle: { fontSize: 16, fontWeight: "bold", marginLeft: 12 },
  statusSubtitle: { fontSize: 12, color: "#475569", marginTop: 8 },
  ...
  emergencyBtnSub: { color: "#fee2e2", fontSize: 12, marginTop: 4 },
  ...
  incidentDesc: { color: "#64748b", fontSize: 11, lineHeight: 18 },
});
